...
* Read `README.md` and report the commands listed there, keeping the distinction between repository-based commands
  (rendered first) and direct commands (no repository, no rendering)
* Also report the own flags and the commands that are intentionally not wrapped, if they are relevant to the question
