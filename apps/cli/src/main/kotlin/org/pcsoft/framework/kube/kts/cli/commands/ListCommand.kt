...
class ListCommand : BaseDirectHelmCommand(), HelmArgsProvider {
    ...
    @Option(
        names = ["--deployed"],
        description = ["$HELM_MARKER show deployed releases. If no other is specified, this will be automatically enabled"]
    )
    ...
    @Option(
        names = ["--filter"],
        description = ["$HELM_MARKER a regular expression (Perl compatible). Any releases that match the expression will be included in the results"],
        paramLabel = "REGEXP"
    )
    ...
    @Option(
        names = ["-m", "--max"],
        description = ["$HELM_MARKER maximum number of releases to fetch"],
        paramLabel = "INT"
    )
    ...
    @Option(
        names = ["--no-headers"],
        description = ["$HELM_MARKER don't print headers when using the default output format"]
    )
    private var noHeaders: Boolean = false

    @Option(
        names = ["--offset"],
        description = ["$HELM_MARKER next release index in the list, used to offset from start value"],
        paramLabel = "INT"
    )
    ...
    @Option(
        names = ["-l", "--selector"],
        description = ["$HELM_MARKER selector (label query) to filter on, supports '=', '==', and '!='"],
        paramLabel = "SELECTOR"
    )
    ...
    @Option(
        names = ["--time-format"],
        description = ["$HELM_MARKER format time using golang time formatter"],
        paramLabel = "FORMAT"
    )
    ...
    @Option(
        names = ["--uninstalled"],
        description = ["$HELM_MARKER show uninstalled releases (if 'helm uninstall --keep-history' was used)"]
    )
    private var uninstalled: Boolean = false

    @Option(
        names = ["--uninstalling"],
        description = ["$HELM_MARKER show releases that are currently being uninstalled"]
    )
