...
class RollbackCommand : BaseDirectHelmCommand(), HelmArgsProvider {
    @Parameters(
        index = "0",
        paramLabel = "RELEASE",
        description = ["Name of the release to roll back (forwarded to helm as positional RELEASE)"]
    )
    ...
    @Parameters(
        ...
        paramLabel = "REVISION",
        arity = "0..1",
        description = ["Revision to roll back to. If omitted, Helm rolls back to the previous revision."]
    )
    ...
    @Option(
        ...
        description = ["$HELM_MARKER allow deletion of new resources created in this rollback when rollback fails"]
    )
    ...
    @Option(
        names = ["--history-max"],
        description = ["$HELM_MARKER limit the maximum number of revisions saved per release. Use 0 for no limit"],
        paramLabel = "INT"
    )
    ...
    @Option(
        names = ["--recreate-pods"],
        description = ["$HELM_MARKER performs pods restart for the resource if applicable"]
    )
    ...
    @Option(
        names = ["--wait"],
        description = ["$HELM_MARKER if set, will wait until all resources are in a ready state before marking the release as successful"]
    )
    private var wait: Boolean = false

    @Option(
        names = ["--wait-for-jobs"],
        description = ["$HELM_MARKER if set and --wait enabled, will wait until all Jobs have been completed before marking the release as successful"]
    )
