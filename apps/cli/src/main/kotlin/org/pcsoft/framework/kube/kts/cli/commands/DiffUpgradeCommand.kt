...
class DiffUpgradeCommand : BaseRenderedHelmCommand(), HelmArgsProvider {
    @Option(
        names = ["--name"],
        description = ["Name of the release (forwarded to helm-diff as positional RELEASE)"],
        paramLabel = "NAME"
    )
    ...
    @Option(
        names = ["--detailed-exitcode"],
        description = ["$HELM_MARKER return a non-zero exit code (2) when there are changes"]
    )
    private var detailedExitcode: Boolean = false

    @Option(
        names = ["--context"],
        description = ["$HELM_MARKER output NUM lines of context around changes (-1 for full context)"],
        paramLabel = "NUM"
    )
    ...
    @Option(
        names = ["--reset-values"],
        description = ["$HELM_MARKER reset the values to the ones built into the chart and merge in any new values"]
    )
    private var resetValues: Boolean = false

    @Option(
        names = ["--reuse-values"],
        description = ["$HELM_MARKER reuse the last release's values and merge in any new values"]
    )
    private var reuseValues: Boolean = false

    @Option(
        names = ["--normalize-manifests"],
        description = ["$HELM_MARKER normalize manifests before running diff to exclude style differences from the output"]
    )
