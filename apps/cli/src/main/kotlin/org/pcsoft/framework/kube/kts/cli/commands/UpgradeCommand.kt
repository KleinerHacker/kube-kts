...
class UpgradeCommand : BaseRenderedHelmCommand(), HelmArgsProvider {
    @Option(
        names = ["--name"],
    ...
    @Option(
        names = ["-i", "--install"],
        description = ["$HELM_MARKER if a release by this name doesn't already exist, run an install"]
    )
    private var install: Boolean = false

    @Option(
        names = ["--atomic"],
        description = ["$HELM_MARKER if set, upgrade process rolls back changes made in case of failed upgrade"]
    )
    private var atomic: Boolean = false

    @Option(
        names = ["--cleanup-on-fail"],
        description = ["$HELM_MARKER allow deletion of new resources created in this upgrade when upgrade fails"]
    )
    private var cleanupOnFail: Boolean = false

    @Option(
        names = ["--create-namespace"],
        description = ["$HELM_MARKER if --install is set, create the release namespace if not present"]
    )
    ...
    @Option(
        names = ["--history-max"],
        description = ["$HELM_MARKER limit the maximum number of revisions saved per release (0 for no limit)"],
        paramLabel = "INT"
    )
    ...
    @Option(
        names = ["-o", "--output"],
        description = ["$HELM_MARKER prints the output in the specified format (table, json, yaml)"],
        paramLabel = "FORMAT"
    )
    ...
    @Option(
        names = ["--reset-values"],
        description = ["$HELM_MARKER when upgrading, reset the values to the ones built into the chart"]
    )
    private var resetValues: Boolean = false

    @Option(
        names = ["--reuse-values"],
        description = ["$HELM_MARKER when upgrading, reuse the last release's values and merge in any overrides"]
    )
    private var reuseValues: Boolean = false

    @Option(
        names = ["--reset-then-reuse-values"],
        description = ["$HELM_MARKER when upgrading, reset the values to the ones built into the chart, apply the last release's values and merge in any overrides"]
    )
    private var resetThenReuseValues: Boolean = false

    @Option(
        names = ["--take-ownership"],
        description = ["$HELM_MARKER if set, upgrade will ignore the check for helm annotations and take ownership of existing resources"]
    )
    private var takeOwnership: Boolean = false

    @Option(
        names = ["--wait"],
        description = ["$HELM_MARKER wait until all Pods, PVCs, Services and Deployments are in a ready state before marking the release as successful"]
    )
    private var wait: Boolean = false

    @Option(
        names = ["--wait-for-jobs"],
        description = ["$HELM_MARKER if set and --wait enabled, will wait until all Jobs have been completed"]
    )
