...
class PullCommand : BaseDirectHelmCommand(), HelmArgsProvider {
    @Parameters(
        index = "0",
        paramLabel = "CHART",
        description = ["Chart reference to download (forwarded to helm as positional CHART)"]
    )
    ...
    @Option(
        names = ["-d", "--destination"],
        description = ["$HELM_MARKER location to write the chart. If this and untardir are specified, untardir is appended to this"],
        paramLabel = "DIR"
    )
    ...
    @Option(
        names = ["--prov"],
        description = ["$HELM_MARKER fetch the provenance file, but don't perform verification"]
    )
    private var prov: Boolean = false

    @Option(
        names = ["--untar"],
        description = ["$HELM_MARKER if set to true, will untar the chart after downloading it"]
    )
    private var untar: Boolean = false

    @Option(
        names = ["--untardir"],
        description = ["$HELM_MARKER if untar is specified, this flag specifies the name of the directory into which the chart is expanded"],
        paramLabel = "DIR"
    )
