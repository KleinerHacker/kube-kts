...
class InstallCommand : BaseRenderedHelmCommand(), HelmArgsProvider {
    ...
    @Option(
        names = ["--atomic"],
        description = ["$HELM_MARKER if set, the installation process deletes the installation on failure"]
    )
    ...
    @Option(
        names = ["--replace"],
        description = ["$HELM_MARKER re-use the given name, only if that name is a deleted release which remains in the history"]
    )
