 ...
 *
 * Renders the KTS repository to Helm YAML and then executes `helm dependency build .` against the
 * rendered chart. The REPOSITORY (and optional TARGET) positionals are inherited from
 * [BaseRenderedHelmCommand].
 */
@Command(name = "build", description = ["Rebuild the charts/ directory based on the Chart.lock file with helm"])
class DependencyBuildCommand : BaseRenderedHelmCommand(), HelmArgsProvider {
    @Mixin
