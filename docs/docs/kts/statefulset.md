...
The `statefulSet` DSL is used to configure Kubernetes **StatefulSet** resources. A StatefulSet manages a set of Pods
with stable, unique network identities and stable, persistent per-Pod storage — suitable for stateful applications such
as databases or message brokers, as opposed to the interchangeable Pods managed by a [Deployment](deployment.md).
...
The following is a comprehensive example showing replicas, the governing service, Pod management, an update strategy,
per-Pod persistent storage via `volumeClaimTemplates` and a claim retention policy.
...
!!! note "Stable storage"
Each entry of `volumeClaimTemplates` provisions one PersistentVolumeClaim **per Pod**. The claims keep their identity
across rescheduling, giving every replica its own durable storage.
