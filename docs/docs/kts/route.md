...
The `route` DSL is used to configure **OpenShift Route** resources, which expose a service under an external host name
and optionally terminate TLS at the OpenShift router.

!!! danger "Not a standard Kubernetes resource"
`Route` (`apiVersion: route.openshift.io/v1`) is **not** part of standard Kubernetes. It is specific to **OpenShift /
OKD** (or compatible distributions that ship the OpenShift router) and will not work on a vanilla Kubernetes cluster.
...
The following is a comprehensive example showing path-based routing, weighted traffic splitting via alternate backends,
