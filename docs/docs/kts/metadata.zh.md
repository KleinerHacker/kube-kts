...
`metadata` DSL 用于定义 Kubernetes 资源的元数据。几乎所有 Kubernetes 资源（如
Service、Ingress、Deployment）都包含元数据，用于标识和组织这些资源。

## 基本用法

在 Kube KTS 中，`metadata` 块定义在某个资源内部（例如 `service`、`ingress`）。资源的 **名称**作为第一个参数传递给 `metadata`
函数。
