...
`job` DSL은 Kubernetes **Job** 리소스를 구성하는 데 사용됩니다. Job은 하나 이상의 Pod를 생성하여 완료될 때까지 실행합니다. [Deployment](deployment.md)가 관리하는
장기 실행 서비스와 달리 배치 및 완료 실행형 워크로드에 적합합니다.

!!! warning "보안: 임포트 제한"
기본적으로 KTS 스크립트는 `import` 문이나 정규화된 클래스 이름 (예: `java.lang.Runtime`)을 **허용하지 않습니다**. 사전 구성된 기본 임포트를 통해 제공되는 타입만 사용할 수 있습니다.
...
다음은 병렬성, 완료 수, 재시도 처리, 자동 정리, 명시적 셀렉터, Pod 실패 정책 및 성공 정책을 보여주는 종합 예제입니다.
...
!!! note "restartPolicy"
Deployment와 달리 Job의 Pod 템플릿은 `restartPolicy`를 `Never` 또는 `OnFailure`로 설정해야 **합니다**. Job에서는 `Always`를 사용할 수 없습니다.
