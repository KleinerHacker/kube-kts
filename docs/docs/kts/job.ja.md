...
完了するまで実行します。[Deployment](deployment.md) が管理する長時間稼働サービスとは異なり、バッチや 完了実行型のワークロードに適しています。
...
以下は、並列度、完了数、リトライ処理、自動クリーンアップ、明示的なセレクター、Pod 障害ポリシー、成功ポリシーを 示す包括的な例です。
...
!!! note "restartPolicy"
Deployment とは異なり、Job の Pod テンプレートでは `restartPolicy` を `Never` または `OnFailure` に 設定する
**必要があります**。Job では `Always` は使用できません。
