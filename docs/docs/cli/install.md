...
Renders the repository to a Helm chart and then runs `helm install` to deploy it into a Kubernetes cluster as a new
release. This is the command that actually changes cluster state, so it talks to the API server using your kube-context
(or the overrides below) and forwards every supported Helm install flag.
...
3. Helm creates the release; the command's exit code mirrors Helm's result. On failure nothing is rolled back unless you
   pass `--atomic`.
...
shorthand is intentionally **not** the name — it is reserved for `--namespace`, to stay in sync with Helm. Omit `--name`
only together with `--generate-name`.
