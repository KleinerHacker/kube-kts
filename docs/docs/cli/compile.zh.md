...
的内存对象模型（chart、deployment、service 等），渲染器随后会将其转换为 YAML。这是确认脚本能否 **编译并执行**
正确的合适命令——包括类型错误、空安全违规以及脚本内部抛出的运行时异常——同时尚不写入 任何输出。

由于编译会运行你的脚本，脚本级问题也会在此最先暴露：无效的值查找、失败的 `require(...)`，或引用了 不允许导入的类的脚本。
