...
聚合基于外部 **`helm-diff` 插件** 的 diff 子命令。它们通过渲染 KTS 仓库并将结果与集群对比，预览某操作将应用的变更。每个子命令都
**需要仓库**，并先运行完整的 *扫描 → 编译 → 渲染* 流水线。不带子命令调用 `diff` 会打印用法列表。
...
!!! note "发布名称通过 `--name` 传入"
由于 `REPOSITORY`/`TARGET` 已占用位置参数，发布名称通过 `--name` 传入，并作为位置参数 `RELEASE` 转发给插件。`-n` 短选项保留给
`--namespace`。
...
`diff upgrade` 接受 [`--set` 系列](install.md) 以及通过 [`-f`/`--values`](index.md#values) 的值文件，它们同时供 Kotlin
脚本与插件使用。
