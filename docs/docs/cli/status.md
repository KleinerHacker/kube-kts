...
Runs `helm status` to show the status of an already installed release. Unlike the rendering commands, `status` operates
on a release that already lives in the cluster, so it needs **neither a repository nor a render step** — the call is
forwarded directly to Helm.
...
The release name is passed as the first positional argument (`RELEASE`) and forwarded to Helm unchanged. The `-n`
shorthand is reserved for `--namespace`.

!!! note "No repository needed"
Because `status` does not render anything, you do not point it at a Kube KTS repository. This is the first command built
on the render-less `BaseDirectHelmCommand` base class.
