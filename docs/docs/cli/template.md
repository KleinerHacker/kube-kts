...
Renders the repository to a Helm chart and then runs `helm template` to print the fully rendered Kubernetes manifests to
stdout. Unlike [`install`](install.md), it does **not** contact or change a cluster (unless you opt in with
`--validate`), which makes it the safest way to see exactly what would be applied. It is ideal for diffing changes,
feeding the output into `kubectl apply`/GitOps, or debugging how your values affect the result.
...
shorthand is reserved for `--namespace`, matching Helm. Use `--generate-name` if you do not care about the name.

!!! tip "Local vs. cluster-aware rendering"
By default templating is fully offline. Add `--validate` to check the manifests against the live cluster, or `-a`/
`--kube-version` to pin the API capabilities used during rendering.
