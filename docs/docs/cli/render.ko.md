...
저장소를 디스크의 표준 Helm Chart 로 렌더링합니다. 스캔·컴파일·실행의 전체 파이프라인을 수행하고, 결과를 일반 Helm Chart (`Chart.yaml`, `values.yaml`, 순수 YAML 로 된
`templates/` 디렉터리)로 대상 디렉터리에 기록합니다. 출력에는 Kotlin 도 Go 템플릿 로직도 없으므로 `helm` 으로 바로 사용하거나 커밋, 차이 비교, 수동 확인이 가능합니다.

`render` 는 Kube KTS 세계와 표준 도구 사이의 다리입니다. Helm 기반 명령이 내부에서 하는 모든 일은 바로 이 단계에서 시작됩니다. 명시적으로 실행하는 것이 Kube KTS 의 산출물을 *보는*
가장 좋은 방법입니다.

!!! tip "출력 확인"
명시적 `TARGET` 을 전달하면 명령 종료 후에도 Chart 가 유지됩니다. 지정하지 않으면 Chart 는 운영체제가 정리할 수 있는 임시 디렉터리에 기록됩니다.
