...
OCI 레지스트리 인증을 관리하는 `helm registry …` 하위 명령을 묶습니다. 이들은 로컬 레지스트리 구성에 대해 동작하므로 **저장소도 렌더링 단계도 필요하지 않습니다** — 호출은 Helm으로 직접
전달됩니다. 하위 명령 없이 `registry`를 호출하면 사용법 목록이 출력됩니다.
