...
将仓库渲染为 Helm Chart，然后对其运行 `helm lint`。Lint 会检查渲染后的 Chart 是否存在问题和违反最佳 实践之处——缺失的必填字段、格式错误的
`Chart.yaml`、无法产生有效 YAML 的模板、建议但缺失的元数据 等等——并以信息、警告或错误的形式报告。
...
这些用于配置与集群的连接和 Helm 环境，并原样转发给 Helm。（对 `lint` 而言，与集群相关的选项通常无关 紧要，因为 lint
是离线的，但为保持一致仍可接受。）
