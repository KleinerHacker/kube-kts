...
차트 정보를 표시하는 `helm show …` 하위 명령 (별칭 `inspect`)을 묶습니다. 이들은 원격/로컬 차트를 대상으로 동작하므로 **저장소도 렌더링 단계도 필요하지 않습니다** — 호출은 Helm으로
직접 전달됩니다. 하위 명령 없이 `show`를 호출하면 사용법 목록이 출력됩니다.
...
각 하위 명령은 [차트 다운로드 옵션](pull.md#차트-다운로드-옵션)(`--repo`, `--version`, 자격 증명, TLS, `--verify` 등)을 받습니다. 또한 `show values`는 다음을
받습니다:
