...
2. **Helm 단계(위임):** Helm 기반 명령(`lint`, `template`, `install`, `uninstall`)의 경우, Kube
...
!!! note "Helm 설치 필요"
    `lint`, `template`, `install`, `uninstall` 은 `helm` 실행 파일이 `PATH` 에 있어야 합니다.
...
- **`TARGET`** 은 렌더링 명령(`render`, `lint`, `template`, `install`, `uninstall`)이 사용하는
...
모든 렌더링 명령(`render`, `lint`, `template`, `install`, `uninstall`)은 값 파일을 받습니다. 값은 두
