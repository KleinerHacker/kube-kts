...
설치된 릴리스의 확장 정보를 다운로드하는 `helm get …` 하위 명령을 묶습니다. 각 하위 명령은 기존 릴리스를 대상으로 동작하므로 **저장소도 렌더링 단계도 필요하지 않습니다** — 호출은 Helm으로 직접
전달됩니다. 하위 명령 없이 `get`을 호출하면 사용법 목록이 출력됩니다.
...
`--revision`은 특정 리비전을 선택하고, `values`의 `-a`/`--all`은 계산된 모든 값을 덤프하며, `--template`은 Go 템플릿을 적용하고, `-o`/`--output`은
`table`/`json`/`yaml`을 선택합니다.
