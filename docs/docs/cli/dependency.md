...
Groups the `helm dependency …` sub-commands (alias `dep`) that manage a chart's dependencies. Each sub-command **needs a
repository**: it runs the full *scan → compile → render* pipeline first and then operates on the rendered chart
(`helm dependency <sub> .`). Invoking `dependency` without a sub-command prints the usage listing.
...
These commands accept values files via [`-f`/`--values`](index.md#values), which feed the Kotlin scripts during
rendering.
