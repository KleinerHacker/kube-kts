...
저장소를 렌더링한 다음 `helm uninstall` 을 실행하여 클러스터에서 하나 이상의 릴리스를 제거합니다. 제거는 릴리스에 속한 리소스를 삭제하고 기본적으로 그 히스토리도 삭제합니다. 다른 Helm 기반 명령과
마찬가지로 kube-context 를 통해 클러스터와 통신하고 지원되는 모든 Helm uninstall 플래그를 전달합니다.

## 동작 방식

1. 저장소를 스캔·컴파일·렌더링합니다 (렌더링 단계는 다른 명령과 워크플로를 통일하기 위함이며, 렌더링된 Chart 가 작업 디렉터리로 사용됩니다).
...
제거는 파괴적입니다. 먼저 `--dry-run` 으로 미리 보고, 나중에 확인하거나 롤백하기 위해 히스토리를 남기려면 `--keep-history` 를 사용하세요.
