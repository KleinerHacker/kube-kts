...
聚合 `helm get …` 子命令，用于下载已安装发布的扩展信息。每个子命令都作用于已存在的发布，因此
**既不需要仓库，也不需要渲染步骤**——调用会直接转发给 Helm。不带子命令调用 `get` 会打印用法列表。
...
`--revision` 选择特定修订版；`values` 上的 `-a`/`--all` 转储所有计算后的值；`--template` 应用 Go 模板；`-o`/`--output` 选择
`table`/`json`/`yaml`。
