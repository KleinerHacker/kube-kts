...
Renders the repository to a Helm chart and then runs `helm lint` against it. Linting examines the rendered chart for
problems and best-practice violations — missing required fields, malformed
`Chart.yaml`, templates that do not produce valid YAML, recommended-but-absent metadata, and so on — and reports them as
information, warnings or errors.

Because Kube KTS renders first, `lint` validates the *actual* chart that Helm would receive, with your values already
applied. It is the natural next step after [`compile`](compile.md): compile proves the scripts run, lint proves the
resulting chart is sound.
...
By default `helm lint` only fails on errors. Add `--strict` to also fail on warnings, which is what you usually want in
CI.
...
These configure the connection to the cluster and the Helm environment. They are forwarded to Helm unchanged. (For
`lint`, cluster-related options rarely matter because linting is offline, but they are accepted for consistency.)
