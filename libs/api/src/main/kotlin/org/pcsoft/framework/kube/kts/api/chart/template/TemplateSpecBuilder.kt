...
/**
 * Builder class for creating instances of [ExplicitTemplateSpec]. This class provides methods to
 ...
 */
class ExplicitTemplateSpecBuilder<S, B> internal constructor(
    private val apiVersion: String,
    private val kind: String,
    private val specBuilder: B
) : TemplateSpecBuilder<S, ExplicitTemplateSpec<S>> where S : ResourceSpec, B : ResourceSpecBuilder<S> {
    ...
    override fun build(): ExplicitTemplateSpec<S> {
        require(metadataBuilder != null) { "Metadata is required" }

        return ExplicitTemplateSpec(apiVersion, kind, metadataBuilder!!.build(), specBuilder.build())
    }
}

/**
 * Builder class for creating instances of [FlatTemplateSpec]. This class provides methods to
 * configure the metadata and specification of a Kubernetes resource template while ensuring
 * that all necessary components are set before the final build.
 *
 * Some values are required.
 ...
 * @param specBuilder An instance of the builder responsible for creating the resource specification.
 */
class FlatTemplateSpecBuilder<S, B> internal constructor(
    private val apiVersion: String,
    private val kind: String,
    private val specBuilder: B
) : TemplateSpecBuilder<S, FlatTemplateSpec<S>> where S : ResourceSpec, B : ResourceSpecBuilder<S> {
    private var metadataBuilder: MetadataTemplateSpecBuilder? = null
    ...
    override fun build(): FlatTemplateSpec<S> {
        require(metadataBuilder != null) { "Metadata is required" }

        return FlatTemplateSpec(apiVersion, kind, metadataBuilder!!.build(), specBuilder.build())
    }
}