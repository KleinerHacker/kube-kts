...
class KubeMixRepositoryTest {
    ...
    fun testSuccessfully() {
        ...
        Assertions.assertEquals(0, ktsRepo.legacyFiles.filter { it.type == KubeFile.Type.CHART }.size)
        Assertions.assertEquals(1, ktsRepo.legacyFiles.filter { it.type == KubeFile.Type.TEMPLATE }.size)
        ...
        Assertions.assertEquals(0, helmRepo.legacyFiles.filter { it.type == KubeFile.Type.CHART }.size)
        Assertions.assertEquals(1, helmRepo.legacyFiles.filter { it.type == KubeFile.Type.TEMPLATE }.size)
